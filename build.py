...
<header class="site-header">
  ...
  <div class="header-top">
    <div class="header-inner">
      ...
      <p class="header-tagline"><span class="tag-gem">◆</span> 부산 16개 구·군 방문 관리 <span class="tag-gem">◆</span> 24시간 상담</p>
...
<footer class="site-footer">
  <div class="container footer-grid">
    <div class="footer-col footer-about">
      ...
      <p class="footer-desc">부산 16개 구·군 방문 출장마사지·홈타이 안내 사이트입니다. 모든 서비스는 안내된 관리 범위와 위생·안전 기준 안에서만 제공됩니다.</p>
      <address class="footer-contact">
        ...
        <span class="footer-contact-row"><span class="footer-label">서비스 지역</span> 부산광역시 16개 구·군</span>
      </address>
    </div>
    <nav class="footer-col" aria-label="서비스 안내">
      <p class="footer-title">서비스</p>
      <ul>
        <li><a href="/busan/">지역별 안내</a></li>
        <li><a href="/busan/stations/">역세권 안내</a></li>
        <li><a href="/busan/areas/">생활권 안내</a></li>
    ...
    <nav class="footer-col" aria-label="이용 안내">
      ...
      <ul>
        ...
        <li><a href="/reservation/#fee">추가 이동비 안내</a></li>
    ...
    <nav class="footer-col" aria-label="정책 및 기준">
      ...
      <ul>
        ...
        <li><a href="/check/#prohibited">불법·선정적 서비스 불가</a></li>
        <li><a href="/support/#policy">운영 기준</a></li>
      </ul>
    </nav>
  </div>
    ...
    <div class="container footer-bottom-inner">
      ...
      <div class="footer-cta">
        <a class="footer-tg-btn" href="https://t.me/googleseolab" target="_blank" rel="noopener nofollow">
          <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M21.94 4.6 18.7 19.9c-.24 1.08-.88 1.34-1.78.84l-4.92-3.63-2.37 2.28c-.26.26-.48.48-.99.48l.35-5.02 9.13-8.25c.4-.35-.09-.55-.62-.2L6.2 13.07l-4.86-1.52c-1.06-.33-1.08-1.06.22-1.57l19-7.32c.88-.33 1.65.2 1.38 1.94z"/></svg>
          웹사이트 제작문의
        </a>
        <a class="footer-tg-btn" href="https://t.me/googleseolab" target="_blank" rel="noopener nofollow">
          <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M21.94 4.6 18.7 19.9c-.24 1.08-.88 1.34-1.78.84l-4.92-3.63-2.37 2.28c-.26.26-.48.48-.99.48l.35-5.02 9.13-8.25c.4-.35-.09-.55-.62-.2L6.2 13.07l-4.86-1.52c-1.06-.33-1.08-1.06.22-1.57l19-7.32c.88-.33 1.65.2 1.38 1.94z"/></svg>
...
def _priority(path: str) -> str:
    if path == "":
        return "1.0"
    if path in ("busan/", "busan/stations/", "busan/areas/"):
        return "0.9"
    segs = [s for s in path.split("/") if s]
    if path.startswith("busan/") and len(segs) == 2:
        return "0.8"          # 구·역·생활권 페이지
    if path.startswith("busan/") and len(segs) == 3:
        return "0.6"          # 대표 동·읍·면 페이지
    return "0.7"              # 예약/가이드/확인/고객센터/소개 등
...
def _changefreq(path: str) -> str:
    ...
    segs = [s for s in path.split("/") if s]
    if path.startswith("busan/") and len(segs) == 3:
        return "monthly"      # 동 페이지
    return "weekly"


def build() -> None:
    report = []
    indexed = []  # [{url, title, desc, path}]
    base = BASE_URL.rstrip("/")

    for page in PAGES:
        path = page["path"]  # "" 또는 "busan/jung-gu/.../" 형태
    ...
    # sitemap.xml (lastmod·changefreq·priority 포함 — 크롤 우선순위 신호)
    rows = "\n".join(
        f"  <url><loc>{html.escape(p['url'])}</loc>"
        f"<lastmod>{lastmod}</lastmod>"
        f"<changefreq>{_changefreq(p['path'])}</changefreq>"
        f"<priority>{_priority(p['path'])}</priority></url>"
        for p in indexed
    )
    ...
    with open(os.path.join(ROOT, "rss.xml"), "w", encoding="utf-8") as f:
        f.write(
            ...
            "<channel>\n"
            f"<title>{html.escape(BRAND)} — 부산 출장마사지·홈타이</title>\n"
            f"<link>{base}/</link>\n"
    ...
    # robots.txt (네이버 Yeti·구글봇 명시 + 사이트맵·RSS)
    ...
    # IndexNow 키 파일 — 빙·네이버·얀덱스 소유 확인용 (루트에 공개 발행)
